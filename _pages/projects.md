---
title: "Posts by Year"
permalink: /projects/
layout: posts
author_profile: true
---
---
layout: single
title: "My Projects"
permalink: /projects/
---

## 🔒 Web Shell Deployment Lab

**Description:**  
Simulated exploitation of an insecure file upload vulnerability using DVWA and Metasploitable2 in a secure lab environment. Successfully deployed a PHP web shell and executed post-exploitation enumeration.

[View Report](https://okiyaalvin.github.io/assets/web-shell-lab.pdf){:target="_blank"}

<iframe src="https://okiyaalvin.github.io/assets/web-shell-lab.pdf#toolbar=0" width="100%" height="500px" style="border: none;"></iframe>

---

## 🛡️ SIEM Monitoring with Wazuh

**Description:**  
Home lab simulation of endpoint monitoring using Wazuh and Sysmon. Implemented alerting, custom rules, and basic reporting on Windows and Kali endpoints.

[View Report](https://yourusername.github.io/assets/pdfs/wazuh-lab-report.pdf){:target="_blank"}

<iframe src="https://yourusername.github.io/assets/pdfs/wazuh-lab-report.pdf#toolbar=0" width="100%" height="500px" style="border: none;"></iframe>
